Shader "Hidden/FovGaussianTunerDebug"
{
	Properties{
	...
	SubShader
	{
		...
		Pass
		{
			...
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag_DebugCircle
			...
			//the fragment shader
			fixed4 frag_DebugCircle(v2f i) : SV_TARGET
			{
				// Eye-tracking variables & FoV variables
				float opacity = 1;
				float base_opacity = 1;
				...
				if (L < I)
					opacity = 0;
				else if (I < L && L < I + (O-I)/8)
					opacity = 1;
				else
					opacity = 0;
				
				col = col * (1 - opacity) + _Color * (opacity);
				return col;
			}
			ENDCG
		}
